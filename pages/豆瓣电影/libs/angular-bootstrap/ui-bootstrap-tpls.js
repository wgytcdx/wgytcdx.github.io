  ...
  .directive('uibCollapse', ['$animate', '$q', '$parse', '$injector', function ($animate, $q, $parse, $injector) {
    ...
    return {
      link(scope, element, attrs) {
        ...
        scope.$watch(attrs.uibCollapse, (shouldCollapse) => {
          if (shouldCollapse) {
            collapse();
          } else {
            expand();
...
// Use accordion-heading below an accordion-group to provide a heading containing HTML
  .directive('uibAccordionHeading', () => ({
    ...
    link(scope, element, attrs, accordionGroupCtrl, transclude) {
...
// You must provide the property on the accordion-group controller that will hold the transcluded element
  .directive('uibAccordionTransclude', () => ({
    ...
    link(scope, element, attrs, controller) {
      scope.$watch(() => controller[attrs.uibAccordionTransclude], (heading) => {
        if (heading) {
  ...
  .directive('uibBtnRadio', ['$parse', function ($parse) {
    return {
      ...
      link(scope, element, attrs, ctrls) {
        const buttonsCtrl = ctrls[0]; const
          ngModelCtrl = ctrls[1];
        ...
        // ui->model
        element.on(buttonsCtrl.toggleEvent, () => {
          if (attrs.disabled) {
            return;
          }
        ...
        if (attrs.uibUncheckable) {
          scope.$watch(uncheckableExpr, (uncheckable) => {
...
angular.module('ui.bootstrap.carousel', [])
  ...
  .controller('UibCarouselController', ['$scope', '$element', '$interval', '$timeout', '$animate', function ($scope, $element, $interval, $timeout, $animate) {
    ...
    let currentIndex = -1;
    let currentInterval; let isPlaying; const
      bufferedTransitions = [];
    ...
    self.removeSlide = function (slide) {
      ...
      const bufferedIndex = bufferedTransitions.indexOf(slide);
      if (bufferedIndex !== -1) {
    ...
    self.select = $scope.select = function (nextSlide, direction) {
      const nextIndex = $scope.indexOfSlide(nextSlide);
    ...
    $scope.$watch('noTransition', (noTransition) => {
    ...
    function goNext(slide, index, direction) {
      ...
      angular.extend(slide, { direction, active: true });
      angular.extend(self.currentSlide || {}, { direction, active: false });
      ...
      && slide.$element && self.slides.length > 1) {
        ...
        $animate.on('addClass', slide.$element, (element, phase) => {
          if (phase === 'close') {
            $scope.$currentTransition = null;
            $animate.off('addClass', element);
            if (bufferedTransitions.length) {
              const nextSlide = bufferedTransitions.pop();
              const nextIndex = $scope.indexOfSlide(nextSlide);
              const nextDirection = nextIndex > self.getCurrentIndex() ? 'next' : 'prev';
              clearBufferedTransitions();
...
angular.module('ui.bootstrap.dateparser', [])
  ...
  // Pulled from https://github.com/mbostock/d3/blob/master/src/format/requote.js
      ...
      formatCodeToRegex = [
        {
          key: 'yyyy',
          regex: '\\d{4}',
          apply(value) { this.year = +value; },
        },
        {
          key: 'yy',
          regex: '\\d{2}',
          apply(value) { this.year = +value + 2000; },
        },
        {
          key: 'y',
          regex: '\\d{1,4}',
        ...
        },
        {
          key: 'M!',
          regex: '0?[1-9]|1[0-2]',
          apply(value) { this.month = value - 1; },
        },
        {
          key: 'MMMM',
          regex: $locale.DATETIME_FORMATS.MONTH.join('|'),
          apply(value) { this.month = $locale.DATETIME_FORMATS.MONTH.indexOf(value); },
        },
        {
          key: 'MMM',
          regex: $locale.DATETIME_FORMATS.SHORTMONTH.join('|'),
          apply(value) { this.month = $locale.DATETIME_FORMATS.SHORTMONTH.indexOf(value); },
        },
        {
          key: 'MM',
          regex: '0[1-9]|1[0-2]',
        ...
        },
        {
          key: 'mm',
          regex: '[0-5][0-9]',
          apply(value) { this.minutes = +value; },
        },
        {
          key: 'm',
          regex: '[0-9]|[1-5][0-9]',
        ...
        },
        {
          key: 'sss',
          regex: '[0-9][0-9][0-9]',
          apply(value) { this.milliseconds = +value; },
        },
        {
          key: 'ss',
          regex: '[0-5][0-9]',
          apply(value) { this.seconds = +value; },
        },
        {
          key: 's',
          regex: '[0-9]|[1-5][0-9]',
    ...
    function createParser(format) {
      ...
      if (quoteIndex > -1) {
        ...
        for (let i = quoteIndex; i < format.length; i++) {
          if (inLiteral) {
            ...
            format[i] = '$';
          } else if (format[i] === '\'') { // start of literal
            format[i] = '$';
            regex[i] = '';
            inLiteral = true;
          }
      ...
      angular.forEach(formatCodeToRegex, (data) => {
    ...
    function convertTimezoneToLocal(date, timezone, reverse) {
      reverse = reverse ? -1 : 1;
      const timezoneOffset = timezoneToOffset(timezone, date.getTimezoneOffset());
        ...
        compile(tElement, tAttrs) {
          ...
          function removeScope(e) {
            ...
            if (linkedScopes.length) {
              ...
              angular.forEach(expToData, (data) => {
    ...
    return {
      ...
      scrollbarWidth() {
        if (angular.isUndefined(SCROLLBAR_WIDTH)) {
          const scrollElem = angular.element('<div style="position: absolute; top: -9999px; width: 50px; height: 50px; overflow: scroll;"></div>');
      ...
      positionElements(hostElem, targetElem, placement, appendToBody) {
        ...
        if (placement[2]) {
          const viewportOffset = this.viewportOffset(hostElem);
...
angular.module('ui.bootstrap.datepicker', ['ui.bootstrap.dateparser', 'ui.bootstrap.isClass', 'ui.bootstrap.position'])
  ...
  .controller('UibDatepickerController', ['$scope', '$attrs', '$parse', '$interpolate', '$locale', '$log', 'dateFilter', 'uibDatepickerConfig', '$datepickerSuppressError', 'uibDateParser',
    function ($scope, $attrs, $parse, $interpolate, $locale, $log, dateFilter, datepickerConfig, $datepickerSuppressError, dateParser) {
      ...
      angular.forEach(['formatDay', 'formatMonth', 'formatYear', 'formatDayHeader', 'formatDayTitle', 'formatMonthTitle'], (key) => {
        self[key] = angular.isDefined($attrs[key]) ? $interpolate($attrs[key])($scope.$parent) : datepickerConfig[key];
      });

      // Evaled configuration attributes
      angular.forEach(['showWeeks', 'yearRows', 'yearColumns', 'shortcutPropagation'], (key) => {
      ...
      angular.forEach(['minDate', 'maxDate'], (key) => {
        if ($attrs[key]) {
          watchListeners.push($scope.$parent.$watch($attrs[key], (value) => {
      ...
      angular.forEach(['minMode', 'maxMode'], (key) => {
        if ($attrs[key]) {
          watchListeners.push($scope.$parent.$watch($attrs[key], (value) => {
      ...
      if (angular.isDefined($attrs.initDate)) {
        ...
        watchListeners.push($scope.$parent.$watch($attrs.initDate, (initDate) => {
      ...
      if (angular.isDefined($attrs.ngDisabled)) {
        watchListeners.push($scope.$parent.$watch($attrs.ngDisabled, (disabled) => {
      ...
      || $attrs.dateDisabled && $scope.dateDisabled({ date, mode: $scope.datepickerMode });
      };

      this.customClass = function (date) {
        return $scope.customClass({ date, mode: $scope.datepickerMode });
      };
      ...
      $scope.select = function (date) {
        if ($scope.datepickerMode === self.minMode) {
          let dt = ngModelCtrl.$viewValue ? dateParser.fromTimezone(new Date(ngModelCtrl.$viewValue), ngModelOptions.timezone) : new Date(0, 0, 0, 0, 0, 0, 0);
  ...
  .controller('UibDaypickerController', ['$scope', '$element', 'dateFilter', function (scope, $element, dateFilter) {
    ...
    this.getDates = function (startDate, n) {
      const dates = new Array(n); const current = new Date(startDate); let i = 0; let
        date;
      while (i < n) {
  ...
  .controller('UibYearpickerController', ['$scope', '$element', 'dateFilter', function (scope, $element, dateFilter) {
    ...
    this._refreshView = function () {
      const years = new Array(range); let
        date;
  ...
  .directive('uibDatepicker', () => ({
    ...
    link(scope, element, attrs, ctrls) {
      const datepickerCtrl = ctrls[0]; const
        ngModelCtrl = ctrls[1];
  ...
  .controller('UibDatepickerPopupController', ['$scope', '$element', '$attrs', '$compile', '$parse', '$document', '$rootScope', '$uibPosition', 'dateFilter', 'uibDateParser', 'uibDatepickerPopupConfig', '$timeout', 'uibDatepickerConfig',
    function (scope, element, attrs, $compile, $parse, $document, $rootScope, $position, dateFilter, dateParser, datepickerPopupConfig, $timeout, datepickerConfig) {
      ...
      let datepickerPopupTemplateUrl; let datepickerTemplateUrl; let popupEl; let datepickerEl;
      let ngModel; let ngModelOptions; let $popup; let altInputFormats; const
        watchListeners = [];
      ...
      this.init = function (_ngModel_) {
        ...
        if (datepickerPopupConfig.html5Types[attrs.type]) {
          ...
          attrs.$observe('uibDatepickerPopup', (value, oldValue) => {
        ...
        if (scope.datepickerOptions) {
          angular.forEach(scope.datepickerOptions, (value, option) => {
        ...
        angular.forEach(['minMode', 'maxMode', 'datepickerMode', 'shortcutPropagation'], (key) => {
          if (attrs[key]) {
            const getAttribute = $parse(attrs[key]);
            const propConfig = {
              get() {
            ...
            if (key === 'datepickerMode') {
              const setAttribute = getAttribute.assign;
        ...
        angular.forEach(['minDate', 'maxDate', 'initDate'], (key) => {
          if (attrs[key]) {
            const getAttribute = $parse(attrs[key]);

            watchListeners.push(scope.$parent.$watch(getAttribute, (value) => {
        ...
        angular.forEach(['formatDay', 'formatMonth', 'formatYear', 'formatDayHeader', 'formatDayTitle', 'formatMonthTitle', 'showWeeks', 'startingDay', 'yearRows', 'yearColumns'], (key) => {
      ...
      scope.dateSelection = function (dt) {
        ...
        const date = scope.date ? dateFilter(scope.date, dateFormat) : null; // Setting to NULL is necessary for form validators to function
        element.val(date);
      ...
      scope.disabled = angular.isDefined(attrs.disabled) || false;
      if (attrs.ngDisabled) {
        watchListeners.push(scope.$parent.$watch($parse(attrs.ngDisabled), (disabled) => {
          scope.disabled = disabled;
        }));
      }
...
angular.module('ui.bootstrap.dropdown', ['ui.bootstrap.position'])
  ...
  .controller('UibDropdownController', ['$scope', '$element', '$attrs', '$parse', 'uibDropdownConfig', 'uibDropdownService', '$animate', '$uibPosition', '$document', '$compile', '$templateRequest', function ($scope, $element, $attrs, $parse, dropdownConfig, uibDropdownService, $animate, $position, $document, $compile, $templateRequest) {
    ...
    let templateScope;
    const { appendToOpenClass } = dropdownConfig;
    const { openClass } = dropdownConfig;
    let getIsOpen;
    ...
    scope.$watch('isOpen', (isOpen, wasOpen) => {
      ...
      const openContainer = appendTo || $element;

      $animate[isOpen ? 'addClass' : 'removeClass'](openContainer, appendTo ? appendToOpenClass : openClass).then(() => {
      ...
      if (isOpen) {
        if (self.dropdownMenuTemplateUrl) {
          $templateRequest(self.dropdownMenuTemplateUrl).then((tplContent) => {
            templateScope = scope.$new();
            $compile(tplContent.trim())(templateScope, (dropdownElement) => {
 ...
 */
    ...
    function ($animateCss, $injector, $modalStack) {
      ...
      function linkFn(scope, element, attrs) {
        if (attrs.modalInClass) {
          ...
          scope.$on($modalStack.NOW_CLOSING_EVENT, (e, setIsAsync) => {
    ...
    function ($modalStack, $q, $animate, $animateCss, $document) {
      return {
        ...
        link(scope, element, attrs) {
          ...
          attrs.$observe('modalRender', (value) => {
            if (value === 'true') {
  ...
  .factory('$uibModalStack', ['$animate', '$animateCss', '$document',
    ...
    function ($animate, $animateCss, $document, $compile, $rootScope, $q, $$multiMap, $$stackedMap) {
      ...
      let focusIndex = 0;
      const tababbleSelector = 'a[href], area[href], input:not([disabled]), '
        ...
        if (modal) {
          switch (evt.which) {
            ...
            case 9: {
              ...
              } else if ($modalStack.isFocusInLastItem(evt)) {
      ...
      $get: ['$rootScope', '$q', '$document', '$templateRequest', '$controller', '$uibResolve', '$uibModalStack',
        function ($rootScope, $q, $document, $templateRequest, $controller, $uibResolve, $modalStack) {
          ...
          $modal.open = function (modalOptions) {
            ...
            const templateAndResolvePromise = $q.all([getTemplatePromise(modalOptions), $uibResolve.resolve(modalOptions.resolve, {}, null, null)]);
            ...
            samePromise = promiseChain = $q.all([promiseChain])
              ...
              .then((tplAndVars) => {
                ...
                if (modalOptions.controller) {
                  ...
                  angular.forEach(tplAndVars[1], (value, key) => {
      ...
      create(ctrl, $scope, $attrs) {
        ...
        ctrl.init = function (ngModelCtrl, config) {
          ...
          if ($attrs.itemsPerPage) {
            ctrl._watchers.push($scope.$parent.$watch($parse($attrs.itemsPerPage), (value) => {
...
angular.module('ui.bootstrap.pager', ['ui.bootstrap.paging'])
  ...
  .directive('uibPager', ['uibPagerConfig', function (uibPagerConfig) {
    return {
      ...
      replace: true,
      link(scope, element, attrs, ctrls) {
        const paginationCtrl = ctrls[0]; const
          ngModelCtrl = ctrls[1];
 ...
 */
   ...
   */
    function snake_case(name) {
      const regexp = /[A-Z]/g;
      const separator = '-';
      return name.replace(regexp, (letter, pos) => (pos ? separator : '') + letter.toLowerCase());
    }
    ...
    this.$get = ['$window', '$compile', '$timeout', '$document', '$uibPosition', '$interpolate', '$rootScope', '$parse', '$$stackedMap', function ($window, $compile, $timeout, $document, $position, $interpolate, $rootScope, $parse, $$stackedMap) {
       ...
       */
        function getTriggers(trigger) {
          ...
          const hide = show.map(trigger => triggerMap[trigger] || trigger);
          return {
            show,
            hide,
          };
        ...
        const template = `<div ${directiveName}-popup `
          ...
          compile(tElem, tAttrs) {
            ...
            return function link(scope, element, attrs, tooltipCtrl) {
              ...
              // check if tooltip exists and is not empty
                ...
                if (!positionTimeout) {
                  ...
                  // Reset the positioning.
                    ...
                    const placement = ttPosition.placement.split('-');
              ...
              // There can only be one tooltip element per directive shown at once.
                ...
                tooltip = tooltipLinker(tooltipLinkedScope, (tooltip) => {
                  if (appendToBody) {
          ...
          scope.$watch($sce.parseAsResourceUrl(attrs.uibTooltipTemplateTransclude), (src) => {
            ...
            if (src) {
              ...
              $templateRequest(src, true).then((response) => {
                ...
                const clone = $compile(template)(newScope, (clone) => {
...
angular.module('ui.bootstrap.popover', ['ui.bootstrap.tooltip'])
  ...
  .directive('uibPopoverTemplatePopup', () => ({
    replace: true,
    scope: {
      title: '@',
      contentExp: '&',
...
angular.module('ui.bootstrap.progressbar', [])
  ...
  .controller('UibProgressController', ['$scope', '$attrs', 'uibProgressConfig', function ($scope, $attrs, progressConfig) {
    ...
    this.addBar = function (bar, element, attrs) {
      ...
      bar.$watch('value', (value) => {
        bar.recalculatePercentage();
      });

      bar.recalculatePercentage = function () {
        const totalPercentage = self.bars.reduce((total, bar) => {
  ...
  .directive('uibBar', () => ({
    ...
    link(scope, element, attrs, progressCtrl) {
...
angular.module('ui.bootstrap.rating', [])
  ...
  .directive('uibRating', () => ({
    ...
    replace: true,
    link(scope, element, attrs, ctrls) {
      const ratingCtrl = ctrls[0]; const
        ngModelCtrl = ctrls[1];
...
angular.module('ui.bootstrap.tabs', [])
  ...
  .directive('uibTabHeadingTransclude', () => ({
    restrict: 'A',
    require: '^uibTab',
    link(scope, elm) {
  ...
  .directive('uibTabContentTransclude', () => {
    return {
      restrict: 'A',
      require: '^uibTabset',
      link(scope, elm, attrs) {
        const tab = scope.$eval(attrs.uibTabContentTransclude);
...
angular.module('ui.bootstrap.timepicker', [])
  ...
  .controller('UibTimepickerController', ['$scope', '$element', '$attrs', '$parse', '$log', '$locale', 'uibTimepickerConfig', function ($scope, $element, $attrs, $parse, $log, $locale, timepickerConfig) {
    ...
    this.init = function (ngModelCtrl_, inputs) {
      ...
      ngModelCtrl.$formatters.unshift(modelValue => (modelValue ? new Date(modelValue) : null));
    ...
    let { hourStep } = timepickerConfig;
    if ($attrs.hourStep) {
      watchers.push($scope.$parent.$watch($parse($attrs.hourStep), (value) => {
        hourStep = +value;
      }));
    }

    let { minuteStep } = timepickerConfig;
    if ($attrs.minuteStep) {
      watchers.push($scope.$parent.$watch($parse($attrs.minuteStep), (value) => {
        minuteStep = +value;
      }));
    }

    let min;
    watchers.push($scope.$parent.$watch($parse($attrs.min), (value) => {
      const dt = new Date(value);
      min = isNaN(dt) ? undefined : dt;
    }));

    let max;
    watchers.push($scope.$parent.$watch($parse($attrs.max), (value) => {
      const dt = new Date(value);
    ...
    let disabled = false;
    if ($attrs.ngDisabled) {
      watchers.push($scope.$parent.$watch($parse($attrs.ngDisabled), (value) => {
        disabled = value;
      }));
    }
    ...
    var { secondStep } = timepickerConfig;
    if ($attrs.secondStep) {
      watchers.push($scope.$parent.$watch($parse($attrs.secondStep), (value) => {
        secondStep = +value;
      }));
    }
    ...
    if ($attrs.showSeconds) {
      watchers.push($scope.$parent.$watch($parse($attrs.showSeconds), (value) => {
    ...
    if ($attrs.showMeridian) {
      watchers.push($scope.$parent.$watch($parse($attrs.showMeridian), (value) => {
        $scope.showMeridian = !!value;

        if (ngModelCtrl.$error.time) {
        // Evaluate from template
          const hours = getHoursFromTemplate(); const
  ...
  .directive('uibTimepicker', ['uibTimepickerConfig', function (uibTimepickerConfig) {
    return {
      ...
      link(scope, element, attrs, ctrls) {
        const timepickerCtrl = ctrls[0]; const
          ngModelCtrl = ctrls[1];
 ...
 */
  ...
  .controller('UibTypeaheadController', ['$scope', '$element', '$attrs', '$compile', '$parse', '$q', '$timeout', '$document', '$window', '$rootScope', '$$debounce', '$uibPosition', 'uibTypeaheadParser',
    function (originalScope, element, attrs, $compile, $parse, $q, $timeout, $document, $window, $rootScope, $$debounce, $position, typeaheadParser) {
      ...
      originalScope.$watch(attrs.typeaheadEditable, (newVal) => {
      ...
      const offDestroy = originalScope.$on('$destroy', () => {
        scope.$destroy();
      });
...
angular.module('ui.bootstrap.carousel').run(() => { !angular.$$csp().noInlineStyle && angular.element(document).find('head').prepend('<style type="text/css">.ng-animate.item:not(.left):not(.right){-webkit-transition:0s ease-in-out left;transition:0s ease-in-out left}</style>'); });
angular.module('ui.bootstrap.datepicker').run(() => { !angular.$$csp().noInlineStyle && angular.element(document).find('head').prepend('<style type="text/css">.uib-datepicker .uib-title{width:100%;}.uib-day button,.uib-month button,.uib-year button{min-width:100%;}.uib-datepicker-popup.dropdown-menu{display:block;}.uib-button-bar{padding:10px 9px 2px;}</style>'); });
angular.module('ui.bootstrap.timepicker').run(() => { !angular.$$csp().noInlineStyle && angular.element(document).find('head').prepend('<style type="text/css">.uib-time input{width:50px;}</style>'); });
angular.module('ui.bootstrap.typeahead').run(() => { !angular.$$csp().noInlineStyle && angular.element(document).find('head').prepend('<style type="text/css">[uib-typeahead-popup].dropdown-menu{display:block;}</style>'); });
