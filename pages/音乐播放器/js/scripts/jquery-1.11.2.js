...
// Pass this if window is not defined yet
  ...
  const { slice } = deletedIds;

  const { concat } = deletedIds;

  const { push } = deletedIds;

  const { indexOf } = deletedIds;
...
(function (window) {
  ...
  const pseudos = `:(${characterEncoding})(?:\\((`
 ...
 */
  ...
  function winnow(elements, qualifier, not) {
    ...
    return jQuery.grep(elements, elem => (jQuery.inArray(elem, qualifier) >= 0) !== not);
  }
 ...
 */
  jQuery.acceptData = function (elem) {
    const noData = jQuery.noData[(`${elem.nodeName} `).toLowerCase()];
    const nodeType = +elem.nodeType || 1;
  ...
  function internalData(elem, name, data, pvt /* Internal Use Only */) {
    ...
    let ret; let thisCache;
    const internalKey = jQuery.expando;
    ...
    const cache = isNode ? jQuery.cache : elem;

    // Only defining an ID for JS objects if its cache already exists allows
    // the code to shortcut on the same path as a DOM node with no cache
    let id = isNode ? elem[internalKey] : elem[internalKey] && internalKey;
  ...
  function internalRemoveData(elem, name, pvt) {
    ...
    const cache = isNode ? jQuery.cache : elem;
    const id = isNode ? elem[jQuery.expando] : jQuery.expando;
  ...
  const rformElems = /^(?:input|select|textarea)$/i;
  const rkeyEvent = /^key/;
 ...
 */
  jQuery.event = {
    ...
    add(elem, types, handler, data, selector) {
      let tmp; let events; let t; let handleObjIn;
      let special; let eventHandle; let handleObj;
      let handlers; let type; let namespaces; let origType;
    ...
    remove(elem, types, handler, selector, mappedTypes) {
      let j; let handleObj; let tmp;
      let origCount; let t; let events;
      let special; let handlers; let type;
      let namespaces; let origType;
      const elemData = jQuery.hasData(elem) && jQuery._data(elem);
    ...
    trigger(event, data, elem, onlyHandlers) {
      let handle; let ontype; let cur;
      let bubbleType; let special; let tmp; let i;
    ...
    dispatch(event) {
      ...
      let i; let ret; let handleObj; let matched; let j;
      let handlerQueue = [];
      const args = slice.call(arguments);
      const handlers = (jQuery._data(this, 'events') || {})[event.type] || [];
    ...
    handlers(event, handlers) {
      let sel; let handleObj; let matches; let i;
      const handlerQueue = [];
    ...
    fix(event) {
      ...
      let i; let prop; let copy;
      const { type } = event;
      const originalEvent = event;
      let fixHook = this.fixHooks[type];
    ...
    mouseHooks: {
      ...
      filter(event, original) {
        let body; let eventDoc; let doc;
        const { button } = original;
        const { fromElement } = original;
    ...
    jQuery.event.special.submit = {
      setup() {
        ...
        jQuery.event.add(this, 'click._submit keypress._submit', (e) => {
          ...
          const elem = e.target;
          const form = jQuery.nodeName(elem, 'input') || jQuery.nodeName(elem, 'button') ? elem.form : undefined;
    ...
    jQuery.each({ focus: 'focusin', blur: 'focusout' }, (orig, fix) => {
      ...
      jQuery.event.special[fix] = {
        setup() {
          ...
          const attaches = jQuery._data(doc, fix);
        ...
        teardown() {
          const doc = this.ownerDocument || this;
          const attaches = jQuery._data(doc, fix) - 1;
  ...
  jQuery.fn.extend({
    ...
    on(types, selector, data, fn, /* INTERNAL */ one) {
      let type; let
        origFn;
  ...
  const rinlinejQuery = / jQuery\d+="(?:null|\d+)"/g;
  const rnoshimcache = new RegExp(`<(?:${nodeNames})[\\s/>]`, 'i');
  ...
  const safeFragment = createSafeFragment(document);
  const fragmentDiv = safeFragment.appendChild(document.createElement('div'));
  ...
  function getAll(context, tag) {
    let elems; let elem;
    let i = 0;
    let found = typeof context.getElementsByTagName !== strundefined ? context.getElementsByTagName(tag || '*')
  ...
  function cloneCopyEvent(src, dest) {
    ...
    let type; let i; let l;
    const oldData = jQuery._data(src);
    const curData = jQuery._data(dest, oldData);
  ...
  jQuery.extend({
    clone(elem, dataAndEvents, deepDataAndEvents) {
      let destElements; let node; let clone; let i; let srcElements;
    ...
    buildFragment(elems, context, scripts, selection) {
      let j; let elem; let contains;
      let tmp; let tag; let tbody; let wrap;
      const l = elems.length;

      // Ensure a safe fragment
      const safe = createSafeFragment(context);
    ...
    cleanData(elems, /* internal */ acceptData) {
      let elem; let type; let id; let data;
      let i = 0;
      const internalKey = jQuery.expando;
      const { cache } = jQuery;
      const { deleteExpando } = support;
  ...
  jQuery.fn.extend({
    ...
    remove(selector, keepData /* Internal Use Only */) {
      let elem;
      const elems = selector ? jQuery.filter(selector, this) : this;
      let i = 0;
    ...
    domManip(args, callback) {
      ...
      let first; let node; let hasScripts;
      let scripts; let doc; let fragment;
      let i = 0;
      const l = this.length;
      const set = this;
      const iNoClone = l - 1;
 ...
 */
  ...
  function actualDisplay(name, doc) {
    let style;
    const elem = jQuery(doc.createElement(name)).appendTo(doc.body);

    // getDefaultComputedStyle might be reliably used only on attached element
    const display = window.getDefaultComputedStyle && (style = window.getDefaultComputedStyle(elem[0]))
 ...
 */
  ...
  if (window.getComputedStyle) {
    ...
    curCSS = function (elem, name, computed) {
      let left; let rs; let rsLeft; let ret;
      const { style } = elem;
  ...
  (function () {
    ...
    let div; let style; let a; let pixelPositionVal; let boxSizingReliableVal;
    let reliableHiddenOffsetsVal; let
    ...
    function computeStyleTests() {
      ...
      let div; let body; let container; let
        contents;
  ...
  function showHide(elements, show) {
    let display; let elem; let hidden;
    const values = [];
    let index = 0;
  ...
  function getWidthOrHeight(elem, name, extra) {
    ...
    const isBorderBox = support.boxSizing && jQuery.css(elem, 'boxSizing', false, styles) === 'border-box';
  ...
  if (!support.opacity) {
    jQuery.cssHooks.opacity = {
      ...
      set(elem, value) {
        ...
        const opacity = jQuery.isNumeric(value) ? `alpha(opacity=${value * 100})` : '';
        const filter = currentStyle && currentStyle.filter || style.filter || '';
  ...
  const rfxnum = new RegExp(`^(?:([+-])=|)(${pnum})([a-z%]*)$`, 'i');
  const rrun = /queueHooks$/;
  const animationPrefilters = [defaultPrefilter];
  const tweeners = {
    '*': [function (prop, value) {
      const tween = this.createTween(prop, value);
      const target = tween.cur();
      let parts = rfxnum.exec(value);
      let unit = parts && parts[3] || (jQuery.cssNumber[prop] ? '' : 'px');

      // Starting value computation is required for potential unit mismatches
      let start = (jQuery.cssNumber[prop] || unit !== 'px' && +target)
  ...
  function createTween(value, prop, animation) {
    let tween;
    const collection = (tweeners[prop] || []).concat(tweeners['*']);
    let index = 0;
  ...
  function defaultPrefilter(elem, props, opts) {
    ...
    let prop; let value; let toggle; let tween; let hooks; let oldfire; let display; let checkDisplay;
    const anim = this;
    const orig = {};
    const { style } = elem;
    let hidden = elem.nodeType && isHidden(elem);
    let dataShow = jQuery._data(elem, 'fxshow');
  ...
  function Animation(elem, properties, options) {
    ...
    let stopped;
    let index = 0;
    const { length } = animationPrefilters;
  ...
  (function () {
    ...
    let input; let div; let select; let a; let
      opt;
  ...
  jQuery.extend({
    valHooks: {
      ...
      select: {
        get(elem) {
          let value; let option;
          const { options } = elem;
          const index = elem.selectedIndex;
          const one = elem.type === 'select-one' || index < 0;
  ...
  let nodeHook; let boolHook;
  const { attrHandle } = jQuery.expr;
  const ruseDefault = /^(?:checked|selected)$/i;
  const { getSetAttribute } = support;
  const getSetInput = support.input;
  ...
  jQuery.extend({
    ...
    removeAttr(elem, value) {
      let name; let propName;
      let i = 0;
      const attrNames = value && value.match(rnotwhite);
  ...
  if (!getSetAttribute) {
    ...
    nodeHook = {
      set(elem, value, name) {
        ...
        let ret = elem.getAttributeNode(name);
        if (!ret) {
    ...
    jQuery.valHooks.button = {
      get(elem, name) {
        const ret = elem.getAttributeNode(name);
  ...
  jQuery.fn.extend({
    addClass(value) {
      let classes; let elem; let cur; let clazz; let j; let finalValue;
      let i = 0;
      const len = this.length;
      const proceed = typeof value === 'string' && value;
    ...
    removeClass(value) {
      ...
      let i = 0;
      const len = this.length;
      const proceed = arguments.length === 0 || typeof value === 'string' && value;
    ...
    toggleClass(value, stateVal) {
      ...
      return this.each(function () {
        if (type === 'string') {
          ...
          const classNames = value.match(rnotwhite) || [];
  ...
  jQuery.parseJSON = function (data) {
    ...
    return str && !jQuery.trim(str.replace(rvalidtokens, (token, comma, open, close) => {
 ...
 */
  function ajaxHandleResponses(s, jqXHR, responses) {
    let firstDataType; let ct; let finalDataType; let type;
    const { contents } = s;
    const { dataTypes } = s;
 ...
 */
  ...
  jQuery.extend({
    ...
    ajax(url, options) {
      ...
      let // Cross-domain detection vars
        parts;
      // Loop variable
      let i;
      ...
      const globalEventContext = s.context && (callbackContext.nodeType || callbackContext.jquery)
      ...
      return this.map(function () {
        ...
        .map(function (i, elem) {
          ...
          return val == null
            ? null
            : jQuery.isArray(val)
              ? jQuery.map(val, val => ({ name: elem.name, value: val.replace(rCRLF, '\r\n') }))
 ...
 */
  jQuery.fn.load = function (url, params, callback) {
    ...
    let selector; let response; let type;
    const self = this;
 ...
 */
  ...
  jQuery.fn.extend({
    ...
    offsetParent() {
      return this.map(function () {
        let offsetParent = this.offsetParent || docElem;
