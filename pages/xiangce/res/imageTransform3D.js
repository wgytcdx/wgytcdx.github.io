...
ge1doot.transform3D.Camera = function (setup, func) {
  ...
  this.enableRx = !setup.disableRx;
  this.enableRy = !setup.disableRy;
  this.enableRz = !setup.disableRz;
  this.cmov = false;
  this.cosX = 1;
  this.sinX = 0;
  this.cosY = 1;
...
ge1doot.transform3D.Point.prototype.projection = function () {
  const sw = this.scr.width >> 1;
  const sh = this.scr.height >> 1;
  // ---- 3D coordinates ----
  const nx = this.x - this.camera.x;
  const ny = this.y - this.camera.y;
  const nz = this.z - this.camera.z;
  ...
  const s = this.camera.cosY * nz + this.camera.sinY * u;
...
ge1doot.transform3D.Triangle.prototype.draw = function () {
  if (this.p0.visible || this.p1.visible || this.p2.visible) {
    let dx; let dy; let
      d;
    // ---- centroid ----
    const xc = (this.p0.X + this.p1.X + this.p2.X) / 3;
    const yc = (this.p0.Y + this.p1.Y + this.p2.Y) / 3;
    // ---- clipping ----
    this.ctx.save();
    this.ctx.beginPath();
    dx = xc - this.p0.X;
    ...
    this.ctx.clip();
    // ---- transform ----
    const t0 = this.p2.X - this.p1.X;
    const t1 = this.p1.Y - this.p2.Y;
    const t2 = this.p2.ty * this.p1.X;
    const t3 = this.p1.tx * this.p2.X;
    const t4 = this.p2.ty * this.p1.Y;
    const t5 = this.p1.ty * this.p2.X;
    const t6 = this.p1.ty * this.p2.Y;
    const t7 = this.p2.tx * this.p1.X;
    const t8 = this.p1.tx * this.p2.Y;
    const t9 = this.p2.tx * this.p1.Y;
    this.ctx.transform(
...
ge1doot.transform3D.Image.prototype.triangulate = function (p0, p1, p2, level) {
  ...
  if (level === 0) {
    ...
    // ---- subdivision ----
    let p01 = this.subdivise(p0, p1);
    let p12 = this.subdivise(p1, p2);
    let p20 = this.subdivise(p2, p0);
    // ---- insert new points ----
