...
ge1doot.Pointer = function (setup) {
  ...
  document.addEventListener('MSHoldVisual', (e) => { e.preventDefault(); }, false);
  ...
  document.addEventListener('mousemove', (e) => { self.pointerMove(e); }, false);
  ...
  if (window.navigator.msPointerEnabled) {
    let nContact = 0;
    const myGesture = new MSGesture();
    myGesture.target = this.elem;
    this.elem.addEventListener('MSPointerDown', (e) => {
      if (e.pointerType == e.MSPOINTER_TYPE_TOUCH) {
        myGesture.addPointer(e.pointerId);
        nContact++;
      }
    }, false);
    this.elem.addEventListener('MSPointerOut', (e) => {
      if (e.pointerType == e.MSPOINTER_TYPE_TOUCH) {
        nContact--;
      }
    }, false);
    this.elem.addEventListener('MSGestureHold', (e) => {
      e.preventDefault();
    }, false);
    this.elem.addEventListener('MSGestureChange', (e) => {
      if (nContact > 1) {
  ...
  if (window.addEventListener) {
    this.elem.addEventListener('DOMMouseScroll', (e) => {
      if (e.preventDefault) e.preventDefault();
  ...
  this.elem.onmousewheel = function () {
    self.wheelDelta = -event.wheelDelta * 0.25;
